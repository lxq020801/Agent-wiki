...
当前版本为 **v0.4.5**，重点解决“把内容可靠地收进知识库”以及“让陌生用户能独立安装和运行”这两件事：用户通过统一的 `./agent-wiki` 命令安装和管理本地服务，在 Chrome 扩展中按首次引导完成配置，再提交抖音内容或选择导入自己的 GitHub Stars，最终在 Obsidian 知识库中得到结构清楚、来源可追溯的 Markdown。
