...
当前版本为 **v0.4.1**，重点解决“把内容可靠地收进知识库”以及“让陌生用户能独立安装和运行”这两件事：用户通过统一的 `./agent-wiki` 命令安装和管理本地服务，在 Chrome 扩展中按首次引导完成配置，再提交抖音内容或选择导入自己的 GitHub Stars，最终在 Obsidian 知识库中得到结构清楚、来源可追溯的 Markdown。
...
资产 frontmatter（文档头部元数据）会记录来源 URL、来源类型、平台 ID 等可获得的信息。普通来源直接写入 `知识资产/`，GitHub 项目写入 `知识资产/GitHub项目/`，并同步更新知识库根目录的 `index.md`。只有派生资产真实生成后，父子资产之间才会建立链接。
