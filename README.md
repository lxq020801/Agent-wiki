...
当前版本为 **v0.4.8**，重点解决“把内容可靠地收进知识库”以及“让陌生用户能独立安装和运行”这两件事：用户通过统一的 `./agent-wiki` 命令安装和管理本地服务，在 Chrome 扩展中按首次引导完成配置，再提交抖音内容或选择导入自己的 GitHub Stars，最终在 Obsidian 知识库中得到结构清楚、来源可追溯的 Markdown。
...
| 来源 | 当前入口 | 生成结果 | 使用前需要 |
|---|---|---|---|
| 抖音视频 | Chrome 扩展提交当前页面或分享链接，并为本次任务选择 1/2/5 FPS（默认 1 FPS） | 视频来源资产；全程使用所选固定 FPS，超过 `1250` 帧安全目标时机械切片并汇总 | 用户自行配置 Ark API Key、同步已登录的抖音 Cookie，并安装 `ffmpeg` / `ffprobe` |
...
资产 frontmatter（文档头部元数据）会记录来源 URL、来源类型、平台 ID 等可获得的信息。无论是直接入库、GitHub Stars 导入还是从已有来源派生，新资产都直接写入 `知识资产/`，使用相同的三段式正文，并同步更新知识库根目录的 `index.md`。形成方式和来源只在 `ingest_intent`、`source_media`、来源 URL、仓库字段与 `derived_from` 等元数据中标识；只有新资产真实生成后，父子资产之间才会建立链接。
...
- 在任务面板查看进度、结果、派生候选与诊断信息；完成后到知识库的 `index.md` 或对应资产目录阅读 Markdown。候选不会写进 Markdown，只有已真实生成的子资产会以“相关资产”链接出现。
