...
def _heuristic_scores(name: str, target_type: str, action: str, item_type: str, context: str) -> dict[str, int]:
    ...
    explicit_github_derivation = target_type == "github_project" and bool(
        re.search(r"github|repo|repository|仓库|开源", text, re.I)
        and re.search(r"派生|沉淀|入库|分析|拆解|验证", text, re.I)
    )
...
def _heuristic_confidence(name: str, target_type: str, action: str, item_type: str, context: str) -> float:
    text = f"{name} {target_type} {action} {item_type} {context}".lower()
    if target_type == "github_project" and re.search(r"github|repo|repository|仓库|开源", text, re.I):
        if re.search(r"派生|沉淀|入库|分析|拆解|验证", text, re.I):
            return 0.82
    return 0.55
