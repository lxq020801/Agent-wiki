...
REQUIRED_SOURCE_SECTIONS = ("简洁概括", "完整内容整理", "ai分析")
...
def validate_source_sections(value: Any) -> str:
    """Validate the canonical source body shared by manual and derived GitHub ingest."""
    text = str(value or "").strip()
    matches = list(re.finditer(r"(?m)^##\s+(.+?)\s*$", text))
    headings = [re.sub(r"\s+", "", match.group(1)).casefold() for match in matches]
    if tuple(headings) != REQUIRED_SOURCE_SECTIONS:
        raise GitHubAssetPipelineError(
            "asset_sections_invalid",
            "模型输出必须依次且仅包含简洁概括、完整内容整理和 AI 分析三个二级章节。",
        )
    for index, match in enumerate(matches):
        ...
        if not text[match.end():end].strip():
            raise GitHubAssetPipelineError(
                "asset_sections_invalid",
                f"模型输出的 {match.group(1).strip()} 章节为空。",
            )
    return text
...
def _preserved_relationship_sections(text: str) -> str:
    """Keep visible relationship metadata when a managed derived asset is refreshed."""
    matches = list(re.finditer(r"(?m)^##\s+(.+?)\s*$", str(text or "")))
    preserved: list[str] = []
    for index, match in enumerate(matches):
        heading = re.sub(r"\s+", "", match.group(1)).casefold()
        if heading not in {"被引用", "相关资产"}:
            continue
        end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
...
class GitHubAssetPipeline:
    ...
    def write(
        ...
        *,
        ...
        model_text = validate_source_sections(model_text)
        tools = _ingest_tools()
        ...
        parent_source_url = _frontmatter_value(existing_text, "parent_source_url")
        relationship_sections = _preserved_relationship_sections(existing_text)
        derived_metadata = ""
        if ingest_intent == "derived_ingest":
            derived_metadata = "derived_kind: github_project\n"
            if parent_source_url:
                derived_metadata += f'parent_source_url: "{_yaml_escape(parent_source_url)}"\n'
        relationship_tail = f"\n\n{relationship_sections}" if relationship_sections else ""
