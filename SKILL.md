---
name: agent-wiki
description: "给 Agent 看的当前运行说明：记录已经实现的本地运行、抖音与 GitHub 入库、首次配置、知识库写入方式和安全边界；产品方向以 PROJECT_INTENT.md 为准。"
---
...
当前实现以 Agent 为主要执行面，本地服务负责持久任务、下载、分析、写库和索引，Chrome 扩展负责首次配置、Cookie、任务入口、GitHub 操作和状态等辅助能力。
...
当用户在 Agent 会话里发来抖音链接时，统一按“知识入库”路径走。扩展或 Agent 只提交来源线索，执行层固定记录 `ingest_intent = knowledge_ingest`，同一来源生成一份独立的来源知识资产。

1. 首次使用先提示用户在仓库根目录运行 `./agent-wiki install` 和 `./agent-wiki start`；安装不会默认注册开机启动。
2. 扩展首次配置固定按 Agent、Ark API、知识库、抖音 Cookie、GitHub（可选）的顺序完成。发现缺 API Key、Cookie 或 OAuth 时，不要让用户把秘密贴进聊天，只提示去扩展中由用户自己配置或授权。
3. 知识库未连接时，让用户点击“选择知识库”，通过 macOS 系统文件夹选择窗口直接选择目标文件夹；不要猜测或改写路径。
4. Agent 收到抖音链接后调用仓库内的入库执行层；这是 Agent 内部实现入口，不应让普通用户把 Python 模块命令当成首选操作方式。
5. 工具链自动下载、分析、直接写入 `知识资产/` 并更新 `index.md`；不自动初始化、暂存或提交 vault Git。
6. 最后只回用户：写入路径、结果摘要和派生状态。真实模型调用、抖音访问与入库结果必须由用户自己的账号、Cookie、网络和额度完成。

## 资产模型

当前采用双轴模型：

- **来源维度**：当前正式用户链路包括 `douyin_video`、`douyin_image_post` 和公开 GitHub 仓库
- **资产用途维度**：当前正式产物包括 `knowledge_asset` 和 `github_project`
...
首次握手自动扫描有效身份标记。只有唯一匹配的 Agent-wiki 知识库可以自动重连；普通目录、旧 vault 和仅存在于 `config.toml` 的路径不会自动连接或写入。用户通过系统原生文件夹选择窗口直接选择知识库文件夹：有效身份库直接激活，空目录原地补齐最小结构，非空未标记目录确认后只创建缺失的索引、必要目录和 `.agent-wiki-vault.json`，不覆盖、复制、迁移或删除已有内容。

## 控制面

安装与启动 WebSocket 控制服务统一使用仓库根入口：
...
服务管理器只有在 PID、私有元数据、进程启动标识、Python 路径和服务入口全部一致时才发送信号；端口被未知进程占用时只报告，不尝试清理。`./agent-wiki doctor` 可检查本地环境，且不会读取 Cookie 内容或 `.obsidian/` 内容。

macOS 开机启动默认关闭，只能由用户显式管理：
...
`autostart disable` 不会停止当前服务。`./agent-wiki uninstall` 只停止身份验证通过的服务，并移除确定由 Agent-wiki 管理的启动接线；知识库和 `~/.agent-wiki/` 中的数据、凭据引用、日志、缓存、任务、诊断及扩展副本全部保留。
...
| 扩展 -> Agent | `vault_scan` | 握手后扫描有效身份库并安全重连唯一匹配项 |
| 扩展 -> Agent | `vault_select_folder` / `vault_select_confirm` | 打开系统文件夹选择器并选择知识库；仅非空未标记目录需要同流程确认 |
...
仓库内的 `scripts/ingest_url.py` 是 Agent 调用的抖音链接执行入口，不是普通用户的公开 CLI。根级 `./agent-wiki` 当前只公开安装、服务生命周期、诊断、自启动和卸载，不提供 `ingest` 子命令。
...
5. 按 `SCHEMA.md` 直接写入 vault 的 `知识资产/`
6. 更新 `index.md`
...
3. 用户需要终端操作时只给根级 `./agent-wiki` 运行命令；不要让用户编辑配置文件或直接运行内部 Python 模块
4. 不要把扩展写成主产品
5. 不要把历史资料当当前口径
