...
def mock_analysis(_material: dict, cleaned_readme: str, _intent: str) -> str:
    embedded_readme = cleaned_readme.replace("\n## ", "\n### ")
    return (
        "## 简洁概括\n"
...
"""
    ...
    def test_shared_section_validator_rejects_extra_or_empty_sections(self) -> None:
        with self.assertRaisesRegex(GitHubAssetPipelineError, "三个二级章节"):
            validate_source_sections(
                "## 简洁概括\n概括\n\n## 完整内容整理\n整理\n\n"
                "## AI 分析\n分析\n\n## README 原文\n不应出现"
            )
        with self.assertRaisesRegex(GitHubAssetPipelineError, "章节为空"):
            validate_source_sections(
                "## 简洁概括\n概括\n\n## 完整内容整理\n\n## AI 分析\n分析"
            )

    def test_managed_derived_refresh_preserves_lineage_and_visible_backlink(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            vault = root / "vault"
            ...
            existing = (
                ...
                'parent_source_url: "https://example.invalid/parent"\n'
                'derived_from: ["[[parent|Parent]]"]\nrelated: ["[[parent|Parent]]"]\n---\n\n'
                "# Existing\n\n## 简洁概括\n旧概括\n\n## 完整内容整理\n旧整理\n\n"
                "## AI 分析\n旧分析\n\n## 被引用\n\n- [[parent|Parent]]：派生资产\n"
            )
            asset.write_text(existing, encoding="utf-8")
            pipeline = GitHubAssetPipeline(config_path=root / "config.toml", analyzer=mock_analysis)

            pipeline.write(
                material(readme="# Updated\n\nCurrent source."),
                vault_path=vault,
                ingest_intent="derived_ingest",
                derived_from=["[[parent|Parent]]"],
                asset_path=asset,
            ...
            self.assertIn("github_managed: true", refreshed)
            self.assertIn("derived_kind: github_project", refreshed)
            self.assertIn('parent_source_url: "https://example.invalid/parent"', refreshed)
            self.assertIn('derived_from: ["[[parent|Parent]]"]', refreshed)
            self.assertIn("## 被引用\n\n- [[parent|Parent]]：派生资产", refreshed)
