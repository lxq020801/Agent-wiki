...
async function main() {
  ...
  const submission = vm.runInContext(
    `submitDouyinIngestTask(${JSON.stringify(candidate)}, { source: 'extension_popup', videoFps: 2 }, {})`,
    context
  );
  ...
  assert.equal(payload.videoFps, 2);
  ...
  assert.throws(
    () => vm.runInContext('normalizeVideoIngestFps(3)', context),
    /只支持 1、2、5 FPS/
  );
