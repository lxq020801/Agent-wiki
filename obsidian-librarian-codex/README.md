...
- `doubao-seed-2-0-mini-260428`：视频拆解工具的策略模型，只负责长视频最高 `1fps` 的动态全片概览、分段 fps 决策和概览 JSON 修复。
...
- 先用全片最高 `1fps` 的动态 fps 做概览，提取粗内容、粗时间线和分段精拆策略
- 如果视频长到官方最低 `0.2fps` 也会超过 `1250` 帧安全目标，会跳过全片概览并记录原因，直接按 `5fps` 分片精拆兜底
