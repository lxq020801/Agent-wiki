...
```text
~/.obsidian-librarian/derived-actions/{parent_task_id}.json
```

每次派生还会写运行审计节点，方便回看程序为什么派生、为什么过滤、为什么选中某个目标：

```text
~/.obsidian-librarian/run-artifacts/{task_id}/05-derive/
~/.obsidian-librarian/run-artifacts/{child_task_id}/05-derive-executor/
```
