...
`knowledge_ingest` 会让主分析模型输出结构化派生候选，例如 GitHub 项目、官方文档、API 文档或需要多源核验的网页研究。执行层不会机械派生，也不会默认自动执行子任务；它会按评分机制筛选、去重、限流后生成候选。

评分维度包括知识价值、父资产依赖度、证据强度、可执行性、时效核验必要性、新颖度、资产适配度、成本风险和歧义度。默认保留最多 `8` 个候选；所有线索都只进入候选态，不存在自动执行开关。
...
正文只展示人类可读的候选摘要表。完整评分、证据、去重状态、验收标准和父资产追溯信息都在系统记录 JSON 里，避免污染正式知识资产 frontmatter。
...
- `doubao-seed-2-0-mini-260428`：视频拆解工具的策略模型，只负责长视频 `1fps` 全片/分片概览、分段 fps 决策和概览 JSON 修复。
