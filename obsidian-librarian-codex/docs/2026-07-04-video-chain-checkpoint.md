...
- 长视频基础切片已加入：
  ...
  - 当前片长：`240s`。
  - 当前重叠：`10s`。
  - 当前流程：mini 先做最高 `1fps` 的动态全片概览和分段策略；策略坏了最多修一次；分片默认 `2` 路并发上传和分析，最后做文本汇总。
...
1. 对 `>600s` 视频先做一次最高 `1fps` 的动态全片概览。
2. 概览不只是分类，还要粗略拆出：
   - 视频大概说了什么。
   - 粗时间线和章节。
   - 重要概念、例子、结论。
...
- `duration > 600s` 时进入长视频模式，先用最高 `1fps` 的动态 fps 做内部概览。
- 超长视频定义为全片用官方最低 `0.2fps` 概览仍会超过 `1250` 帧安全目标，也就是约 `6250s / 104m10s` 以上。
...
- 策略修复、修复失败和 fps 上调记录在 `~/.obsidian-librarian/logs/video-strategy-events.jsonl`。
