...
人工确认、忽略和自动入队状态写入 `~/.obsidian-librarian/derived-actions/{parent_task_id}.json`，再由 WebSocket 状态快照合并回 `derivedTasks`。只有通过可见候选闸门的项目才进入这里，避免把 UI 操作状态和泛研究想法塞进父资产 Markdown。
...
- 路径：`~/.obsidian-librarian/run-artifacts/{task_id}/`
