...
候选不设固定数量上限，不再用数值评分、置信度或“必须是唯一主对象”作为可见/执行门槛。结构无效、URL 不安全和重复资产仍会被拦截；缺少目标或匹配有歧义时进入 `needs_target`，不会直接消失。

GitHub 候选不强制要求视频里出现链接；如果只有项目名，派生工具会用 GitHub API 搜索仓库、读取 README，并把 README/描述与视频上下文对齐。派生请求会在内存中复用 macOS Keychain 的 GitHub 登录，跨子任务串行节流，并对限流或服务端错误做有限自动重试；凭证不写入状态或审计。能唯一高置信匹配时自动补全目标并执行；匹配不唯一时降级为人工补链接。

完整候选记录写入 runtime `run-artifacts/`，不作为普通入库的额外 vault 文件。GitHub 名称能被官方 API 唯一可信解析时可自动继续；明确安全 URL 的官方文档和网页候选也可自动入队。
...
`05-derive/` 记录分析正文输入、JSON 候选、Markdown fallback 候选、归一化候选、最终保留/过滤结果和公开投影；`05-derive-executor/` 记录派生子任务输入、GitHub/网页目标解析、来源材料、模型 prompt、原始输出、清洗后输出、写库结果和父子链接结果。
...
- 质量档、目标帧配置、FPS 范围和自适应模式：均已从正式配置移除
- 视频时长超过 `250` 秒，即 `时长 × 5fps > 1250`，才做机械切片

机械切片：

- 切片长度固定为 `1250 / 5 = 250` 秒，相邻片重叠 `10s`，下一片起点步长为 `240s`
