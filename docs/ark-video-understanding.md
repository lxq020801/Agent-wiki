...
3. 使用用户为本次任务选择的固定 `1/2/5 FPS`，不做本地画面变化预扫描或动态 FPS 决策。
...
- 模型上传 FPS：每个任务固定选择 `1`、`2` 或 `5 FPS`，默认 `1 FPS`；任务执行中不自动变档。
- 不运行本地预扫描；旧的全局质量档、自适应 FPS 范围和 `video_fps_mode` 仍不恢复。
- 方舟硬上限：约 `1280` 帧。
- 项目安全目标：`1250` 帧。
- 切片长度固定为 `1250 / FPS`：1 FPS 为 1250 秒，2 FPS 为 625 秒，5 FPS 为 250 秒。
- 相邻切片重叠：`10` 秒。
- 相邻切片起点步长：`1250 / FPS - 10` 秒。
- 同一个视频的所有切片使用同一 FPS。
- 切片并发：默认 `2`，可配置为 `1-4`。
